...
function moveRange (elem) {
	...
	var block2 = {}
	if (elem.classList.contains('priceSlider__block-min')) {
	...
	function onMouseMove(e) {
    	...
    	let rangeMax = 40000;
        if(f==0){
          value =  (newLeft / (parent.coords.width / (rangeMax - rangeMin)) + rangeMin).toFixed() + `<span class="priceSlider__rub">₽<span>`;
        } else {
          value = (newLeft / (parent.coords.width / (rangeMax - rangeMin))+ 0.3 + rangeMin).toFixed() + `<span class="priceSlider__rub">₽<span>`;
        }
    	...
    	indicator.style.bottom = parseFloat(window.getComputedStyle(elem).getPropertyValue('bottom')) - 23 +"px";
    	...
    	if (value <= rangeMin){
    		...
    		// indicator.innerHTML = `<span class="priceSlider__value">${value}</span>`;
